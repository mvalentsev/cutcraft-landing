...
## Цель
Модернизация landing page с Tailwind 4.1 и Vite 7, устранение копипасты, улучшение производительности и доступности.
...
- Создали utility классы: `.feature-card`, `.payment-card`, `.model-card`, `.cta-button`
...
**Почему БЕЗ @apply:**
- Tailwind 4.1 не позволяет `@apply card-glass` внутри того же `@layer components`
...
**Изменения:**
- Добавлен `const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches`
...
**Проверка:**
- ✅ Build проходит
- ✅ Логика корректная (если prefers-reduced-motion → сразу все видимо, без анимаций)
...
**Изменения:**
- Заменили 15+ повторений длинных классов на utility (`.feature-card`, `.payment-card`, `.model-card`, `.cta-button`)
...
**Проверка:**
- ✅ `git rev-parse --show-toplevel` → `/home/michael/projects/cutcraft2/pages-landing`
...
- `cssMinify: 'lightningcss'` избыточен (@tailwindcss/vite уже использует LightningCSS)
...
**Дата:** 2025-10-09
**Время:** ~2 часа (включая исправление ошибок)
**Выводы:** ВСЕГДА проверять дефолты ПЕРЕД добавлением конфига!
